          ...
          {WHY_CARDS.map((card, idx) => {
            ...
            return (
              <Link
                key={idx}
                href={card.href}
                className="group relative flex flex-col justify-between rounded-3xl border border-slate-200 bg-white p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:border-emerald-300"
              >
                <div>
                  <div className={`flex size-12 items-center justify-center rounded-2xl border ${card.iconBg} p-2.5 mb-6 shadow-sm`}>
                    <Icon className={`size-6 ${card.color} transition-transform group-hover:scale-110`} />
                  </div>
                ...
                <div className="mt-8 pt-4 border-t border-slate-100 flex items-center gap-1.5 text-xs font-bold text-slate-700 group-hover:text-emerald-600 transition-colors">
